...
library(pROC)
library(foreach)
load("data2003ca.rdata")
names(data2003ca) <- gsub(" ", "", names(data2003ca))
data2003ca$UPB = log(data2003ca$UPB)
default = data2003ca[which(data2003ca$def_flag == TRUE),]
constant_col = which(colMeans(default)==0)
data2003ca = data2003ca[,-constant_col]

set.seed(70)
index = sample.int(10, size = nrow(data2003ca), replace = T)
#auc_2 = matrix(0,nrow = 2, ncol = 20)

loopfunction = function(k){
  auc_2 = matrix(0,nrow = 2, ncol = 1)
  data = data2003ca[index != k,]
  test = data2003ca[index == k,]
  
  default = data[which(data$def_flag==1),]   
  nondefault = data[-which(data$def_flag==1),]   
  ...
  f <- as.formula(paste("class ~ 1|", paste(names(default)[-8], collapse = " + ")));f
  ...
  while((abs((newQ-oldQ)/oldQ)>0.0002 | j < 20) & (j < 51)){  
    ...
    design_matrix = cbind(rep(1,nrow(data)), data[,-8])
    ...
    new_mldata = mlogit::mlogit.data(new_data, choice = "class",shape = "wide")
    f <- as.formula(paste("class ~ 1|", paste(names(default)[-8], collapse = " + ")))
    new_mlogit.model<-mnlogit(f , data = new_mldata, reflevel="c1")  
    newQ = logLik(new_mlogit.model)
    coef = as.numeric(coef(new_mlogit.model))
  ...
  f <- as.formula(paste("class ~ 1|", paste(names(cluster)[-c(8,ncol(cluster))], collapse = " + ")))
  ...
  auc_2[1,1] = as.numeric(auc(roc(response = test$def_flag, predictor = result)))
  
  glm.fit = glm(def_flag ~. , data=data, family=binomial)
  result = predict(glm.fit, newdata=test, probability=TRUE)
  auc_2[2,1] = as.numeric(auc(roc(response = test$def_flag, predictor = result)))
  print(k)
  return(auc_2)
}
...
library(doMC)
library(pROC)
registerDoMC(7)  #change the 7 to your number of CPU cores  
...
loopfunction = function(k){
  ...
  auc_3[1,1] = as.numeric(auc(roc(response = test$def_flag, predictor = result)))
  
  glm.fit = glm(def_flag ~. , data=data, family=binomial)
  result = predict(glm.fit, newdata=test, probability=TRUE)
  auc_3[2,1] = as.numeric(auc(roc(response = test$def_flag, predictor = result)))
  return(auc_3)
}

library(foreach)
library(doMC)
registerDoMC(8)  #change 8 to your number of CPU cores  
...
cv_auc_2003 = rbind(auc_2[2,],auc_2[1,],auc_3[1,])
rowMeans(cv_auc_2003)
